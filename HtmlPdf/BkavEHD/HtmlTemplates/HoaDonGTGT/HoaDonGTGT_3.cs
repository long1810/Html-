...
namespace BES
{
    public class HoaDonGTGT_3 : HoaDonBanHang
    {
        public string br_HD3 = "#f9a4a4";
        public string bg_HD3 = "#d98282";
        public string bg2_HD3 = "#e9e0cc";
        public HoaDonGTGT_3()
        {
            CSS = new HtmlCSS();
            CSS.style = $@"
        ...
        public override string CreateHMLT()
        {
            ...
            html.Append($"<div style='display: grid; grid-template-columns: 0.5fr 1.3fr 1fr; gap: 10px; border: 1px solid {br_HD3};padding:10px;background-color: {br_HD3};'>");
            ...
            // TaxCODE
            html.Append($"<div style=' border: 1px solid {br_HD3};line-height: 1.4; padding:5px; border-top: none;'>");
            ...
            //Buyer
            html.Append($"<div style=' border: 1px solid {br_HD3};line-height: 1.4; padding:5px; border-top: none; border-bottom: none;'>");
            ...
            AddItemHeadTable(html, br_HD3, "STT (No.)", "Tên hàng hóa, dịch vụ (Description)", "ĐVT (Unit)", "SL (Quantity)", "Đơn giá (Unit Price)", "Thành tiền (Amount)");
            AddItemBodyTable(html, "1", "2", "3", "4", "5", "6 = 4 x 5");
            for (int i = 0; i < 15; i++)
            {
                AddItemBodyTable(html, "1", "2.644.545", "2.644.545", "10%", "264.455", "264.455");
            }
            AddItemBodyTable2(html, bg2_HD3, "Cộng tiền hàng (Sub total)", "", 5, 1);
            AddItemBodyTable4(html, bg2_HD3, "Thuế suất GTGT (Tax rate)", "10%", "Cộng tiền thuế GTGT (VAT amount)", "", 2, 3, 1);
            AddItemBodyTable2(html, bg2_HD3, "Tổng cộng tiền thanh toán (Total payment)", "", 5, 1);
            AddItemBodyTable3(html, bg2_HD3, "Số tiền viết bằng chữ (Amount in words)", "", 6);
            html.Append("   </table>");
